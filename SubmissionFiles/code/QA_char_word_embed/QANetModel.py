...
class CQAttention(nn.Module):
    def __init__(self, D, dropout, c_len, q_len):
        super().__init__()
        self.dropout = dropout
        self.c_len = c_len
        self.q_len = q_len
    ...
    def trilinear_for_attention(self, C, Q):
        ...
        subres0 = torch.matmul(C, self.w4C).expand([-1, -1, self.q_len])
        subres1 = torch.matmul(Q, self.w4Q).transpose(1, 2).expand([-1, self.c_len, -1])
...
class QaNet(nn.Module):
    def __init__(self, word_mat, char_mat, D, Dword, Dchar, dropout, dropout_char, Nh, c_len, q_len):
        super().__init__()
        self.dropout = dropout
        self.c_len = c_len
        self.q_len = q_len
        ...
        self.cq_att = CQAttention(D=D, dropout=dropout, c_len = c_len, q_len = q_len)
    ...
    def forward(self, Cwid, Ccid, Qwid, Qcid):
        ...
        C, Q = self.emb(Cc, Cw, self.c_len), self.emb(Qc, Qw, self.q_len)
