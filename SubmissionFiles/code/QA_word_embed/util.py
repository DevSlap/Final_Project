...
def final_xml(pred_dict, eval_path, save_path, num_visuals = 10):
    ...
    np.random.seed(5)
    visual_ids = np.random.choice(list(pred_dict), size=num_visuals, replace=False)
    #visual_ids = list(pred_dict)[:,num_visuals]
    output_array=[]
    with open(eval_path, 'r') as eval_file:
        eval_dict = json.load(eval_file)
    for i, id_ in enumerate(visual_ids):
        ...
        output_array.append(text_string)

        # tbx.add_text(tag='{}/{}_of_{}'.format(split, i + 1, num_visuals),
        #              text_string=tbl_fmt.format(question, context, gold, pred),
        #              global_step=step)

    generate_xml_output(output_array, save_path)
      

def generate_xml_output(output_array, save_path):
    top = Element('QuestionAndAnswerResults')
    comment = Comment('Generated by Swapna Anandaraman')
    top.append(comment)
    
    for i in output_array:
        top_element = SubElement(top, 'example')
        sub_element1=SubElement(top_element, 'QuestionAnswer')
        sub_element1.text = i
